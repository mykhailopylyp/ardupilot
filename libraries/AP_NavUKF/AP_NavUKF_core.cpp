...
*/
/*
 * Covariance prediction: EKF Jacobian (UKF_USE_UT=0) or unscented transform (UKF_USE_UT=1).
 */
void NavUKF_core::CovariancePrediction(Vector3F *rotVarVecPtr)
{
    if (frontend->_ukf_use_ut == 0) {
        CovariancePredictionEKF(rotVarVecPtr);
    } else {
...
void NavUKF_core::CovariancePredictionUT(Vector3F *rotVarVecPtr)
{
    ...
    */
    ...
    const uint8_t n = uint8_t(stateIndexLim + 1);
    ...
    const ftype Wm0 = lambda / n_lambda;
    const ftype Wc0 = Wm0 + (1.0f - ukf_alpha * ukf_alpha + ukf_beta);
    const ftype Wi  = 0.5f / n_lambda;

    // Lower Cholesky L of P into nextP
    ...
    for (uint8_t attempt = 0; attempt < 5 && !chol_ok; attempt++) {
        for (uint8_t i = 0; i < n; i++) {
            for (uint8_t j = 0; j < n; j++) {
                chol_scratch[i][j] = nextP[i][j];
            }
            if (attempt > 0) {
                chol_scratch[i][i] += 1.0e-6f * (ftype)(1u << (attempt - 1));
            }
        }
        chol_ok = choleskyLower(chol_scratch, n);
        if (!chol_ok) {
            // reload from P for next jitter attempt
            for (uint8_t i = 0; i < n; i++) {
                for (uint8_t j = 0; j < n; j++) {
                    nextP[i][j] = P[i][j];
                }
    ...
    for (uint8_t i = 0; i < n; i++) {
        for (uint8_t j = 0; j < n; j++) {
            nextP[i][j] = chol_scratch[i][j];
        }
    }

    const uint8_t n_sigma = uint8_t(2 * n + 1);
    for (uint8_t j = 0; j < n; j++) {
        sigma_prop[0][j] = stateBeforePredict[j];
    }
    // Attitude sigma points are multiplicative (rotation-vector on the
    // quaternion manifold). Additive R^4 draws + renormalise distort P when
    // gamma is O(1); projecting the Cholesky column onto the tangent space
    // then applying q⊗δq keeps UT consistent with unit-quaternion attitude.
    const QuaternionF qnom(stateBeforePredict[0], stateBeforePredict[1],
                           stateBeforePredict[2], stateBeforePredict[3]);
    for (uint8_t i = 0; i < n; i++) {
        ftype dq[4];
        for (uint8_t j = 0; j < 4; j++) {
            const ftype L_ji = (j >= i) ? nextP[j][i] : 0.0f;
            dq[j] = gamma * L_ji;
        }
        // Remove component parallel to q (stay in T_q S^3)
        ...
        // δθ = 2 * Ξ(q)^T δq  (body rotation vector)
        Vector3F dtheta(
        ...
        sigma_prop[i + 1 + n][0] = q_neg[0];
        sigma_prop[i + 1 + n][1] = q_neg[1];
        sigma_prop[i + 1 + n][2] = q_neg[2];
        sigma_prop[i + 1 + n][3] = q_neg[3];

        for (uint8_t j = 4; j < n; j++) {
            const ftype L_ji = (j >= i) ? nextP[j][i] : 0.0f;
            const ftype step = gamma * L_ji;
            sigma_prop[i + 1][j] = stateBeforePredict[j] + step;
            sigma_prop[i + 1 + n][j] = stateBeforePredict[j] - step;
        }
    }
    // Align quaternion signs to the reference before propagation
    {
        const ftype qref[4] = {
            stateBeforePredict[0], stateBeforePredict[1],
            stateBeforePredict[2], stateBeforePredict[3]
        };
        for (uint8_t s = 1; s < n_sigma; s++) {
    ...
    for (uint8_t s = 0; s < n_sigma; s++) {
        ...
        // so FinishFusion (quat Jacobians) still sees quaternion-component P.
        {
    ...
    // destroys cross-covariances and prevents tilt alignment. Mag fusion
    // repairs P reactively when HPH'+R or FinishFusion detects ill-conditioning.
...
void NavUKF_core::CovariancePredictionEKF(Vector3F *rotVarVecPtr)
{
    ftype daxVar;       // X axis delta angle noise variance rad^2
    ftype dayVar;       // Y axis delta angle noise variance rad^2
    ftype dazVar;       // Z axis delta angle noise variance rad^2
