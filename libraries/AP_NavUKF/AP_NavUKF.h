...
class NavUKF {
    friend class NavUKF_core;

public:
    NavUKF();

    /* Do not allow copies */
    CLASS_NO_COPY(NavUKF);
...
private:
    ...
    NavUKF_core *core = nullptr;

    uint32_t _frameTimeUsec;        // time per IMU frame
    uint8_t  _framesPerPrediction;  // expected number of IMU frames per prediction
  
    // values for UKF_LOG_LEVEL
    enum class LogLevel {
        ALL = 0,
        UKF4 = 1,
        UKF4_GSF = 2,
    ...
    AP_Int8  _enable;               // zero to disable UKF
    ...
    AP_Int8  _ukf_use_ut;           // 0: Jacobian cov predict, 1: unscented cov predict
    ...
    AP_Int8 _imuMask;               // Bitmask of IMUs to instantiate UKF for
    ...
    AP_Int8 _gsfRunMask;            // mask controlling which UKF instances run a separate EKF-GSF yaw estimator
    AP_Int8 _gsfUseMask;            // mask controlling which UKF instances will use EKF-GSF yaw estimator data to assit with yaw resets
    AP_Int8 _gsfResetMaxCount;      // maximum number of times the UKF is allowed to reset it's yaw to the EKF-GSF estimate
...
#define FLOW_USE_TERRAIN 2
    ...
    const uint16_t gpsNoFixTimeout_ms = 2000;      // Time without a fix required to reset GPS alignment checks when UKF_OPTIONS bit 0 is set (msec)
