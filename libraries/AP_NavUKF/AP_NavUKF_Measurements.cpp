...
void NavUKF_core::readRangeFinder(void)
{
    uint8_t midIndex;
    uint8_t maxIndex;
    uint8_t minIndex;
...
void NavUKF_core::writeBodyFrameOdom(float quality, const Vector3f &delPos, const Vector3f &delAng, float delTime, uint32_t timeStamp_ms, uint16_t delay_ms, const Vector3f &posOffset)
{
...
void NavUKF_core::writeWheelOdom(float delAng, float delTime, uint32_t timeStamp_ms, const Vector3f &posOffset, float radius)
{
#if UKF_FEATURE_BODY_ODOM
...
void NavUKF_core::writeOptFlowMeas(const uint8_t rawFlowQuality, const Vector2f &rawFlowRates, const Vector2f &rawGyroRates, const uint32_t msecFlowMeas, const Vector3f &posOffset, float heightOverride)
{
...
void NavUKF_core::tryChangeCompass(void)
{
...
void NavUKF_core::tryChangeCompass(uint8_t mag_index)
{
    const auto &compass = dal.compass();
    const uint8_t maxCount = compass.get_count();
    ...
    if (compass.healthy(mag_index) && compass.use_for_yaw(mag_index) && mag_index != magSelectIndex) {
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u switching to compass %u",(unsigned)imu_index,magSelectIndex);
 ...
 *  errors.
 */
void NavUKF_core::readIMUData(bool startPredictEnabled)
{
...
bool NavUKF_core::readDeltaVelocity(uint8_t ins_index, Vector3F &dVel, ftype &dVel_dt) {
    const auto &ins = dal.ins();
...
void NavUKF_core::readGpsData()
{
    ...
    if (frontend->option_is_enabled(NavUKF::Option::JammingExpected) &&
...
void NavUKF_core::readGpsYawData()
{
    const auto &gps = dal.gps();
...
// return false if data is not available
bool NavUKF_core::readDeltaAngle(uint8_t ins_index, Vector3F &dAng, ftype &dAngDT) {
    const auto &ins = dal.ins();
...
void NavUKF_core::calcFiltBaroOffset()
{
...
void NavUKF_core::correctEkfOriginHeight()
{
...
void NavUKF_core::readAirSpdData()
{
...
void NavUKF_core::readRngBcnData()
{
...
void NavUKF_core::writeEulerYawAngle(float yawAngle, float yawAngleErr, uint32_t timeStamp_ms, uint8_t type)
{
    // limit update rate to maximum allowed by sensor buffers and fusion process
    // don't try to write to buffer until the filter has been initialised
...
void NavUKF_core::writeDefaultAirSpeed(float airspeed, float uncertainty)
{
...
void NavUKF_core::writeExtNavData(const Vector3f &pos, const Quaternion &quat, float posErr, float angErr, uint32_t timeStamp_ms, uint16_t delay_ms, uint32_t resetTime_ms)
{
...
void NavUKF_core::writeExtNavVelData(const Vector3f &vel, float err, uint32_t timeStamp_ms, uint16_t delay_ms)
{
#if UKF_FEATURE_EXTERNAL_NAV
...
void NavUKF_core::writeTerrainData(float alt_m)
{
 ...
 */
void NavUKF_core::update_gps_selection(void)
{
    const auto &gps = dal.gps();
 ...
 */
void NavUKF_core::update_mag_selection(void)
{
    const auto &compass = dal.compass();
 ...
 */
void NavUKF_core::update_baro_selection(void)
{
    auto &baro = dal.baro();
 ...
 */
void NavUKF_core::update_airspeed_selection(void)
{
 ...
 */
void NavUKF_core::update_sensor_selection(void)
{
 ...
 */
void NavUKF_core::updateTimingStatistics(void)
{
    if (timing.count == 0) {
...
/*
  ...
  error
 */
void NavUKF_core::learnInactiveBiases(void)
{
...
*/
ftype NavUKF_core::MagDeclination(void) const
{
...
*/
void NavUKF_core::updateMovementCheck(void)
{
    if (!onGround)
    {
    ...
    if (logStatusChange || imuSampleTime_ms - lastMoveCheckLogTime_ms > 200) {
        ...
        /* UKFM logging omitted */
#endif
    }
}

void NavUKF_core::SampleDragData(const imu_elements &imu)
{
 ...
 */
void NavUKF_core::getEarthFieldTable(const Location &loc)
{
 ...
 */
void NavUKF_core::checkUpdateEarthField(void)
{
