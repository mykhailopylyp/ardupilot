...
#define UKF_MAG_FINAL_RESET_ALT 2.5f
#define UKF_MAG_FINAL_RESET_ALT_SUB 0.5f

// learning rate for mag biases when using GPS yaw
#define UKF_GPS_MAG_LEARN_RATE 0.005f

// learning limit for mag biases when using GPS yaw (Gauss)
#define UKF_GPS_MAG_LEARN_LIMIT 0.02f
...
#define UKF_POSXY_STATE_LIMIT 50.0e6
#else
#define UKF_POSXY_STATE_LIMIT 1.0e6
#endif
...
class NavUKF_core : public NavEKF_core_common
{
public:
    // Constructor
    NavUKF_core(class NavUKF *_frontend, class AP_DAL &dal);
...
private:
    ...
    // On failure A is left partially overwritten; caller must restore.
    static bool choleskyLower(ftype A[24][24], uint8_t n);

    // Restore positive-definiteness of an n×n covariance in A without leaving
    // the unscented path (diagonal loading, then diagonal-only as last resort).
    // scratch must be a distinct n×n buffer (sigma_prop[0..23] is used).
    static void forceCovariancePSD(ftype A[24][24], ftype scratch[24][24], uint8_t n);

    // Propagate one full-state sigma point through the strapdown IMU model.
    // delAng/delVel must be RAW IMU samples; biases in x[] are applied here.
    static void propagateSigmaPoint(ftype x[24],
                                    Matrix3F &Tnb,
...
#endif  // if UKF_FEATURE_BEACON_FUSION
    ...
    */
    ...
    void Log_Write_UKF1(uint64_t time_us) const;
    void Log_Write_UKF2(uint64_t time_us) const;
    void Log_Write_UKF3(uint64_t time_us) const;
    void Log_Write_UKF4(uint64_t time_us) const;
    void Log_Write_UKF5(uint64_t time_us) const;
    void Log_Write_UKFS(uint64_t time_us) const;
