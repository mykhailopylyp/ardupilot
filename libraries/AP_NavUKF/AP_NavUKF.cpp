...
#ifndef UKF_PRIMARY_DEFAULT
#define UKF_PRIMARY_DEFAULT 0
#endif
...
const AP_Param::GroupInfo NavUKF::var_info[] = {

    // @Param: ENABLE
    // @DisplayName: Enable EKF3
    // @Description: This enables UKF. Enabling UKF only makes the maths run, it does not mean it will be used for flight control. To use it for flight control set AHRS_EKF_TYPE=4. A reboot or restart will need to be performed after changing the value of UKF_ENABLE for it to take effect.
    // @Values: 0:Disabled, 1:Enabled
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("ENABLE", 0, NavUKF, _enable, 1, AP_PARAM_FLAG_ENABLE),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("VELNE_M_NSE", 2, NavUKF, _gpsHorizVelNoise, VELNE_M_NSE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("VELD_M_NSE", 3, NavUKF, _gpsVertVelNoise, VELD_M_NSE_DEFAULT),

    // @Param: VEL_I_GATE
    // @DisplayName: GPS velocity innovation gate size
    // @Description: This sets the percentage number of standard deviations applied to the GPS velocity measurement innovation consistency check. Decreasing it makes it more likely that good measurements will be rejected. Increasing it makes it more likely that bad measurements will be accepted. If UKF_GLITCH_RAD set to 0 the velocity innovations will be clipped instead of rejected if they exceed the gate size and a smaller value of UKF_VEL_I_GATE not exceeding 300 is recommended to limit the effect of GPS transient errors.
    // @Range: 100 1000
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("VEL_I_GATE", 4, NavUKF, _gpsVelInnovGate, VEL_I_GATE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("POSNE_M_NSE", 5, NavUKF, _gpsHorizPosNoise, POSNE_M_NSE_DEFAULT),

    // @Param: POS_I_GATE
    // @DisplayName: GPS position measurement gate size
    // @Description: This sets the percentage number of standard deviations applied to the GPS position measurement innovation consistency check. Decreasing it makes it more likely that good measurements will be rejected. Increasing it makes it more likely that bad measurements will be accepted. If UKF_GLITCH_RAD has been set to 0 the horizontal position innovations will be clipped instead of rejected if they exceed the gate size so a smaller value of UKF_POS_I_GATE not exceeding 300 is recommended to limit the effect of GPS transient errors.
    // @Range: 100 1000
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("POS_I_GATE", 6, NavUKF, _gpsPosInnovGate, POS_I_GATE_DEFAULT),

    // @Param: GLITCH_RAD
    // @DisplayName: GPS glitch radius gate size (m)
    // @Description: This controls the maximum radial uncertainty in position between the value predicted by the filter and the value measured by the GPS before the filter position and velocity states are reset to the GPS. Making this value larger allows the filter to ignore larger GPS glitches but also means that non-GPS errors such as IMU and compass can create a larger error in position before the filter is forced back to the GPS position. If UKF_GLITCH_RAD set to 0 the GPS innovations will be clipped instead of rejected if they exceed the gate size set by UKF_VEL_I_GATE and UKF_POS_I_GATE which can be useful if poor quality sensor data is causing GPS rejection and loss of navigation but does make the EKF more susceptible to GPS glitches. If setting UKF_GLITCH_RAD to 0 it is recommended to reduce UKF_VEL_I_GATE and UKF_POS_I_GATE to 300.
    // @Range: 10 100
    // @Increment: 5
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("GLITCH_RAD", 7, NavUKF, _gpsGlitchRadiusMax, GLITCH_RADIUS_DEFAULT),

    // 8 previously used for UKF_GPS_DELAY parameter that has been deprecated.
    ...
    // @Description: This is the RMS value of noise in the altitude measurement. Increasing it reduces the weighting of the baro measurement and will make the filter respond more slowly to baro measurement errors, but will make it more sensitive to GPS and accelerometer errors. A larger value for UKF_ALT_M_NSE may be required when operating with UKF_SRCx_POSZ = 0. This parameter also sets the noise for the 'synthetic' zero height measurement that is used when UKF_SRCx_POSZ = 0.
    // @Range: 0.01 100.0
    // @Increment: 0.1
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("ALT_M_NSE", 10, NavUKF, _baroAltNoise, ALT_M_NSE_DEFAULT),

    // @Param: HGT_I_GATE
    // @DisplayName: Height measurement gate size
    // @Description: This sets the percentage number of standard deviations applied to the height measurement innovation consistency check. Decreasing it makes it more likely that good measurements will be rejected. Increasing it makes it more likely that bad measurements will be accepted.  If UKF_GLITCH_RAD set to 0 the vertical position innovations will be clipped instead of rejected if they exceed the gate size and a smaller value of UKF_HGT_I_GATE not exceeding 300 is recommended to limit the effect of height sensor transient errors.
    // @Range: 100 1000
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("HGT_I_GATE", 11, NavUKF, _hgtInnovGate, HGT_I_GATE_DEFAULT),
    ...
    // @Units: ms
    // @RebootRequired: True
    AP_GROUPINFO("HGT_DELAY", 12, NavUKF, _hgtDelay_ms, 60),
    ...
    // @User: Advanced
    // @Units: Gauss
    AP_GROUPINFO("MAG_M_NSE", 13, NavUKF, _magNoise, MAG_M_NSE_DEFAULT),

    // @Param: MAG_CAL
    // @DisplayName: Magnetometer default fusion mode
    // @Description: This determines when the filter will use the 3-axis magnetometer fusion model that estimates both earth and body fixed magnetic field states and when it will use a simpler magnetic heading fusion model that does not use magnetic field states. The 3-axis magnetometer fusion is only suitable for use when the external magnetic field environment is stable. UKF_MAG_CAL = 0 uses heading fusion on ground, 3-axis fusion in-flight, and is the default setting for Plane users. UKF_MAG_CAL = 1 uses 3-axis fusion only when manoeuvring. UKF_MAG_CAL = 2 uses heading fusion at all times, is recommended if the external magnetic field is varying and is the default for rovers. UKF_MAG_CAL = 3 uses heading fusion on the ground and 3-axis fusion after the first in-air field and yaw reset has completed, and is the default for copters. UKF_MAG_CAL = 4 uses 3-axis fusion at all times. UKF_MAG_CAL = 7 uses 3-axis fusion on the ground and after the first in-air field and yaw reset has completed. This allows the magnetic field to be learned on the ground before takeoff (useful when swapping batteries with different magnetic signatures) while inhibiting learning during the initial climb when motor magnetic interference is strongest. NOTE : Use of simple heading magnetometer fusion makes vehicle compass calibration and alignment errors harder for the EKF to detect which reduces the sensitivity of the Copter EKF failsafe algorithm. NOTE: The fusion mode can be forced to 2 for specific EKF cores using the UKF_MAG_MASK parameter. NOTE: limited operation without a magnetometer or any other yaw sensor is possible by setting all COMPASS_USE, COMPASS_USE2, COMPASS_USE3, etc parameters to 0 and setting COMPASS_ENABLE to 0. If this is done, the UKF_GSF_RUN and UKF_GSF_USE masks must be set to the same as UKF_IMU_MASK. A yaw angle derived from IMU and GPS velocity data using a Gaussian Sum Filter (GSF) will then be used to align the yaw when flight commences and there is sufficient movement.
    // @Values: 0:When flying,1:When manoeuvring,2:Never,3:After first climb yaw reset,4:Always,5:Use external yaw sensor (Deprecated in 4.1+ see UKF_SRCn_YAW),6:External yaw sensor with compass fallback (Deprecated in 4.1+ see UKF_SRCn_YAW),7:On ground and after first climb yaw reset
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("MAG_CAL", 14, NavUKF, _magCal, MAG_CAL_DEFAULT),
    ...
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("MAG_I_GATE", 15, NavUKF, _magInnovGate, MAG_I_GATE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("EAS_M_NSE", 16, NavUKF, _easNoise, 1.4f),
    ...
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("EAS_I_GATE", 17, NavUKF, _tasInnovGate, 400),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("RNG_M_NSE", 18, NavUKF, _rngNoise, 0.5f),
    ...
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("RNG_I_GATE", 19, NavUKF, _rngInnovGate, 500),
    ...
    // @User: Advanced
    // @Units: rad/s
    AP_GROUPINFO("FLOW_MAX", 20, NavUKF, _maxFlowRate, 2.5f),
    ...
    // @User: Advanced
    // @Units: rad/s
    AP_GROUPINFO("FLOW_M_NSE", 21, NavUKF, _flowNoise, FLOW_M_NSE_DEFAULT),
    ...
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("FLOW_I_GATE", 22, NavUKF, _flowInnovGate, FLOW_I_GATE_DEFAULT),
    ...
    // @Units: ms
    // @RebootRequired: True
    AP_GROUPINFO("FLOW_DELAY", 23, NavUKF, _flowDelay_ms, FLOW_MEAS_DELAY),
    ...
    // @User: Advanced
    // @Units: rad/s
    AP_GROUPINFO("GYRO_P_NSE", 24, NavUKF, _gyrNoise, GYRO_P_NSE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m/s/s
    AP_GROUPINFO("ACC_P_NSE", 25, NavUKF, _accNoise, ACC_P_NSE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: rad/s/s
    AP_GROUPINFO("GBIAS_P_NSE", 26, NavUKF, _gyroBiasProcessNoise, GBIAS_P_NSE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m/s/s/s
    AP_GROUPINFO("ABIAS_P_NSE", 28, NavUKF, _accelBiasProcessNoise, ABIAS_P_NSE_DEFAULT),

    // 29 previously used for EK2_MAG_P_NSE parameter that has been replaced with UKF_MAGE_P_NSE and UKF_MAGB_P_NSE
    ...
    // @User: Advanced
    // @Units: m/s/s
    AP_GROUPINFO("WIND_P_NSE", 30, NavUKF, _windVelProcessNoise, WIND_P_NSE_DEFAULT),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("WIND_PSCALE", 31, NavUKF, _wndVarHgtRateScale, 1.0f),
    ...
    // @User: Advanced
    AP_GROUPINFO("GPS_CHECK",    32, NavUKF, _gpsCheck, 31),

    // @Param: IMU_MASK
    // @DisplayName: Bitmask of active IMUs
    // @Description: 1 byte bitmap of IMUs to use in EKF3. A separate instance of UKF will be started for each IMU selected. Set to 1 to use the first IMU only (default), set to 2 to use the second IMU only, set to 3 to use the first and second IMU. Additional IMU's can be used up to a maximum of 6 if memory and processing resources permit. There may be insufficient memory and processing resources to run multiple instances. If this occurs UKF will fail to start.
    // @Bitmask: 0:FirstIMU,1:SecondIMU,2:ThirdIMU,3:FourthIMU,4:FifthIMU,5:SixthIMU
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("IMU_MASK",     33, NavUKF, _imuMask, HAL_EKF_IMU_MASK_DEFAULT),
    ...
    // @User: Advanced
    // @Units: %
    AP_GROUPINFO("CHECK_SCALE", 34, NavUKF, _gpsCheckScaler, CHECK_SCALER_DEFAULT),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("NOAID_M_NSE", 35, NavUKF, _noaidHorizNoise, 10.0f),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("BETA_MASK", 36, NavUKF, _betaMask, 0),
    ...
    // @User: Advanced
    // @Units: rad
    AP_GROUPINFO("YAW_M_NSE", 37, NavUKF, _yawNoise, 0.5f),
    ...
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("YAW_I_GATE", 38, NavUKF, _yawInnovGate, 300),
    ...
    // @User: Advanced
    // @Units: cs
    AP_GROUPINFO("TAU_OUTPUT", 39, NavUKF, _tauVelPosOutput, 25),
    ...
    // @User: Advanced
    // @Units: Gauss/s
    AP_GROUPINFO("MAGE_P_NSE", 40, NavUKF, _magEarthProcessNoise, MAGE_P_NSE_DEFAULT),
    ...
    // @User: Advanced
    // @Units: Gauss/s
    AP_GROUPINFO("MAGB_P_NSE", 41, NavUKF, _magBodyProcessNoise, MAGB_P_NSE_DEFAULT),

    // @Param: RNG_USE_HGT
    // @DisplayName: Range finder switch height percentage
    // @Description: Range finder can be used as the primary height source when below this percentage of its maximum range (see RNGFNDx_MAX) and the primary height source is Baro or GPS (see UKF_SRCx_POSZ).  This feature should not be used for terrain following as it is designed for vertical takeoff and landing with climb above the range finder use height before commencing the mission, and with horizontal position changes below that height being limited to a flat region around the takeoff and landing point.
    // @Range: -1 70
    // @Increment: 1
    // @User: Advanced
    // @Units: %
    AP_GROUPINFO("RNG_USE_HGT", 42, NavUKF, _useRngSwHgt, -1),
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("TERR_GRAD", 43, NavUKF, _terrGradMax, 0.1f),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("BCN_M_NSE", 44, NavUKF, _rngBcnNoise, 1.0f),
    ...
    // @Increment: 25
    // @User: Advanced
    AP_GROUPINFO("BCN_I_GTE", 45, NavUKF, _rngBcnInnovGate, 500),
    ...
    // @Units: ms
    // @RebootRequired: True
    AP_GROUPINFO("BCN_DELAY", 46, NavUKF, _rngBcnDelay_ms, 50),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("RNG_USE_SPD", 47, NavUKF, _useRngSwSpd, 2.0f),
    ...
    // @User: Advanced
    // @Units: m/s/s
    AP_GROUPINFO("ACC_BIAS_LIM", 48, NavUKF, _accBiasLim, 1.0f),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("MAG_MASK", 49, NavUKF, _magMask, 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("OGN_HGT_MASK", 50, NavUKF, _originHgtMode, 0),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("VIS_VERR_MIN", 51, NavUKF, _visOdmVelErrMin, 0.1f),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("VIS_VERR_MAX", 52, NavUKF, _visOdmVelErrMax, 0.9f),
    ...
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("WENC_VERR", 53, NavUKF, _wencOdmVelErr, 0.1f),
    ...
    // @RebootRequired: True
    AP_GROUPINFO("FLOW_USE", 54, NavUKF, _flowUse, FLOW_USE_DEFAULT),
    ...
    // @Range: 0.1 30.0
    // @Units: Hz
    AP_GROUPINFO("HRT_FILT", 55, NavUKF, _hrt_filt_freq, 2.0f),
    ...
    // @Range: 0 500
    // @Units: mGauss
    AP_GROUPINFO("MAG_EF_LIM", 56, NavUKF, _mag_ef_limit, 50),

    // @Param: GSF_RUN_MASK
    // @DisplayName: Bitmask of which EKF-GSF yaw estimators run
    // @Description: 1 byte bitmap of which UKF instances run an independent EKF-GSF yaw estimator to provide a backup yaw estimate that doesn't rely on magnetometer data. This estimator uses IMU, GPS and, if available, airspeed data. EKF-GSF yaw estimator data for the primary UKF instance will be logged as GSF0 and GSF1 messages. Use of the yaw estimate generated by this algorithm is controlled by the UKF_GSF_USE_MASK and UKF_GSF_RST_MAX parameters. To run the EKF-GSF yaw estimator in ride-along and logging only, set UKF_GSF_USE to 0. 
    // @Bitmask: 0:FirstEKF,1:SecondEKF,2:ThirdEKF,3:FourthEKF,4:FifthEKF,5:SixthEKF
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("GSF_RUN_MASK", 57, NavUKF, _gsfRunMask, 3),

    // @Param: GSF_USE_MASK
    // @DisplayName: Bitmask of which EKF-GSF yaw estimators are used
    // @Description: A bitmask of which UKF instances will use the output from the EKF-GSF yaw estimator that has been turned on by the UKF_GSF_RUN_MASK parameter. If the inertial navigation calculation stops following the GPS, then the vehicle code can request UKF to attempt to resolve the issue, either by performing a yaw reset if enabled by this parameter by switching to another UKF instance.
    // @Bitmask: 0:FirstEKF,1:SecondEKF,2:ThirdEKF,3:FourthEKF,4:FifthEKF,5:SixthEKF
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("GSF_USE_MASK", 58, NavUKF, _gsfUseMask, 3),
    ...
    // @Description: Sets the maximum number of times the UKF will be allowed to reset its yaw to the estimate from the EKF-GSF yaw estimator. No resets will be allowed unless the use of the EKF-GSF yaw estimate is enabled via the UKF_GSF_USE_MASK parameter.
    // @Range: 1 10
    // @Increment: 1
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("GSF_RST_MAX", 60, NavUKF, _gsfResetMaxCount, 2),

    // @Param: ERR_THRESH
    // @DisplayName: UKF Lane Relative Error Sensitivity Threshold
    ...
    // @Increment: 0.05
    // @User: Advanced
    AP_GROUPINFO("ERR_THRESH", 61, NavUKF, _err_thresh, 0.2),

    // @Param: AFFINITY
    // @DisplayName: UKF Sensor Affinity Options
    ...
    AP_GROUPINFO("AFFINITY", 62, NavUKF, _affinity, 0),

    AP_SUBGROUPEXTENSION("", 63, NavUKF, var_info2),

    AP_GROUPEND
};

// second table of parameters. allows us to go beyond the 64 parameter limit
const AP_Param::GroupInfo NavUKF::var_info2[] = {

    // @Group: SRC
    // @Path: ../AP_NavEKF/AP_NavEKF_Source.cpp
    AP_SUBGROUPINFO(sources, "SRC", 1, NavUKF, AP_NavEKF_Source),

    // @Param: DRAG_BCOEF_X
    // @DisplayName: Ballistic coefficient for X axis drag
    // @Description: Ratio of mass to drag coefficient measured along the X body axis. This parameter enables estimation of wind drift for vehicles with bluff bodies and without propulsion forces in the X and Y direction (eg multicopters). The drag produced by this effect scales with speed squared. Set to a positive value > 1.0 to enable. A starting value is the mass in Kg divided by the frontal area. The predicted drag from the rotors is specified separately by the UKF_DRAG_MCOEF parameter.
    // @Range: 0.0 1000.0
    // @Units: kg/m/m
    // @User: Advanced
    AP_GROUPINFO("DRAG_BCOEF_X", 2, NavUKF, _ballisticCoef_x, 0.0f),

    // @Param: DRAG_BCOEF_Y
    // @DisplayName: Ballistic coefficient for Y axis drag
    // @Description: Ratio of mass to drag coefficient measured along the Y body axis. This parameter enables estimation of wind drift for vehicles with bluff bodies and without propulsion forces in the X and Y direction (eg multicopters). The drag produced by this effect scales with speed squared. Set to a positive value > 1.0 to enable. A starting value is the mass in Kg divided by the side area. The predicted drag from the rotors is specified separately by the UKF_DRAG_MCOEF parameter.
    // @Range: 50.0 1000.0
    // @Units: kg/m/m
    // @User: Advanced
    AP_GROUPINFO("DRAG_BCOEF_Y", 3, NavUKF, _ballisticCoef_y, 0.0f),

    // @Param: DRAG_M_NSE
    // @DisplayName: Observation noise for drag acceleration
    // @Description: This sets the amount of noise used when fusing X and Y acceleration as an observation that enables estimation of wind velocity for multi-rotor vehicles. This feature is enabled by the UKF_DRAG_BCOEF_X and UKF_DRAG_BCOEF_Y parameters
    // @Range: 0.1 2.0
    // @Increment: 0.1
    // @User: Advanced
    // @Units: m/s/s
    AP_GROUPINFO("DRAG_M_NSE", 4, NavUKF, _dragObsNoise, 0.5f),

    // @Param: DRAG_MCOEF
    // @DisplayName: Momentum coefficient for propeller drag
    // @Description: This parameter is used to predict the drag produced by the rotors when flying a multi-copter, enabling estimation of wind drift. The drag produced by this effect scales with speed not speed squared and is produced because some of the air velocity normal to the rotors axis of rotation is lost when passing through the rotor disc which changes the momentum of the airflow causing drag. For unducted rotors the effect is roughly proportional to the area of the propeller blades when viewed side on and changes with different propellers. It is higher for ducted rotors. For example if flying at 15 m/s at sea level conditions produces a rotor induced drag acceleration of 1.5 m/s/s, then UKF_DRAG_MCOEF would be set to 0.1 = (1.5/15.0). Set UKF_MCOEF to a positive value to enable wind estimation using this drag effect. To account for the drag produced by the body which scales with speed squared, see documentation for the UKF_DRAG_BCOEF_X and UKF_DRAG_BCOEF_Y parameters.
    // @Range: 0.0 1.0
    // @Increment: 0.01
    // @Units: 1/s
    // @User: Advanced
    AP_GROUPINFO("DRAG_MCOEF", 5, NavUKF, _momentumDragCoef, 0.0f),

    // @Param: OGNM_TEST_SF
    // @DisplayName: On ground not moving test scale factor
    // @Description: This parameter is adjust the sensitivity of the on ground not moving test which is used to assist with learning the yaw gyro bias and stopping yaw drift before flight when operating without a yaw sensor. Bigger values allow the detection of a not moving condition with noiser IMU data. Check the UKFM data logged when the vehicle is on ground not moving and adjust the value of OGNM_TEST_SF to be slightly higher than the maximum value of the UKFM.ADR, UKFM.ALR, UKFM.GDR and UKFM.GLR test levels.
    // @Range: 1.0 10.0
    // @Increment: 0.5
    // @User: Advanced
    AP_GROUPINFO("OGNM_TEST_SF", 6, NavUKF, _ognmTestScaleFactor, 2.0f),
    ...
    // @Increment: 0.5
    // @User: Advanced
    AP_GROUPINFO("GND_EFF_DZ", 7, NavUKF, _baroGndEffectDeadZone, 4.0f),

    // @Param: PRIMARY
    // @DisplayName: Primary core number
    // @Description: The core number (index in IMU mask) that will be used as the primary EKF core on startup. While disarmed the EKF will force the use of this core. A value of 0 corresponds to the first IMU in UKF_IMU_MASK.
    // @Range: 0 2
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("PRIMARY", 8, NavUKF, _primary_core, UKF_PRIMARY_DEFAULT),

    // @Param: LOG_LEVEL
    // @DisplayName: Logging Level
    // @Description: Determines how verbose the UKF streaming logging is. A value of 0 provides full logging(default), a value of 1 only UKF4 scaled innovations are logged, a value of 2 both UKF4 and GSF are logged, and a value of 3 disables all streaming logging of EKF3.
    // @Range: 0 3
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("LOG_LEVEL", 9, NavUKF, _log_level, 0),
    ...
    // @User: Advanced
    // @Units: m
    AP_GROUPINFO("GPS_VACC_MAX", 10, NavUKF, _gpsVAccThreshold, 0.0f),

    // @Param: OPTIONS
    // @DisplayName: Optional EKF behaviour
    // @Description: EKF optional behaviour. Bit 0 (JammingExpected): Setting JammingExpected will change the EKF behaviour such that if dead reckoning navigation is possible it will require the preflight alignment GPS quality checks controlled by UKF_GPS_CHECK and UKF_CHECK_SCALE to pass before resuming GPS use if GPS lock is lost for more than 2 seconds to prevent bad position estimate. Bit 1 (Manual lane switching): DANGEROUS – If enabled, this disables automatic lane switching. If the active lane becomes unhealthy, no automatic switching will occur. Users must manually set UKF_PRIMARY to change lanes. No health checks will be performed on the selected lane. Use with extreme caution.  Bit 2 (Optflow may use terrain alt): Terrain SRTM data will be used if the vehicle climbs above the rangefinder's range allowing optical flow to be used at higher altitudes. Bit 3 (AGL KF for optflow scaling): Use a 2-state IMU-aided AGL Kalman filter (height + vertical velocity, fused with rangefinder) to compute the height-above-ground used for optical flow velocity scaling, instead of terrainState-pd. This decouples optical flow scaling from errors in the main filter's vertical position state.
    // @Bitmask: 0:JammingExpected, 1:ManualLaneSwitching, 2:Optflow may use terrain alt, 3:AGL KF for optflow scaling
    // @User: Advanced
    AP_GROUPINFO("OPTIONS",  11, NavUKF, _options, 0),

    // @Param: ALPHA
    // @DisplayName: UKF sigma-point alpha
    // @Description: Unscented transform spread parameter. Tuned default 0.35 (with BETA=2, KAPPA=0) beat EKF3 on held-out complex SITL flights; very small alpha (e.g. 0.001) yields huge negative central weights and poor P.
    // @Range: 0.001 1.0
    // @Increment: 0.001
    // @User: Advanced
    AP_GROUPINFO("ALPHA", 12, NavUKF, _ukf_alpha, 0.35f),

    // @Param: BETA
    // @DisplayName: UKF sigma-point beta
    // @Description: Unscented transform prior distribution parameter. Use 2 for Gaussian priors.
    // @Range: 0 10
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("BETA", 13, NavUKF, _ukf_beta, 2.0f),

    // @Param: KAPPA
    // @DisplayName: UKF sigma-point kappa
    // @Description: Secondary unscented transform scaling parameter.
    // @Range: -24 24
    // @Increment: 0.1
    // @User: Advanced
...
bool NavUKF::InitialiseFilter(void)
{
    ...
    if (core == nullptr) {
        ...
        if (dal.available_memory() < sizeof(NavUKF_core)*num_cores + 4096) {
            GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL, "UKF not enough memory");
            _enable.set(0);
            num_cores = 0;
            return false;
        }
        ...
        core = (NavUKF_core*)dal.malloc_type(sizeof(NavUKF_core)*num_cores, AP_DAL::MemoryType::FAST);
        if (core == nullptr) {
            _enable.set(0);
            num_cores = 0;
            GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL, "UKF allocation failed");
            return false;
        }

        // Call constructors on all cores
        for (uint8_t i = 0; i < num_cores; i++) {
            new (&core[i]) NavUKF_core(this, dal);
        }
    }
...
/*
  ...
  core
 */
bool NavUKF::coreBetterScore(uint8_t new_core, uint8_t current_core) const
{
    const NavUKF_core &oldCore = core[current_core];
    const NavUKF_core &newCore = core[new_core];
    if (!newCore.healthy()) {
        ...
        for (uint8_t coreIndex=0; coreIndex<num_cores; coreIndex++) {
            if (coreIndex != primary) {
                ...
                if (altCoreAvailable) {
                    ...
                    const NavUKF_core &newCore = core[coreIndex];
                    const NavUKF_core &oldCore = core[primary];
...
*/
void NavUKF::checkLaneSwitch(void)
{
...
void NavUKF::switchLane(uint8_t new_lane_index)
{
    ...
    if (new_lane_index != primary) {
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL, "UKF lane switch %u", primary);
    }
}

void NavUKF::requestYawReset(void)
{
...
*/
float NavUKF::updateCoreErrorScores()
{
    for (uint8_t i = 0; i < num_cores; i++) {
...
*/
void NavUKF::updateCoreRelativeErrors()
{
    float error = 0;
    for (uint8_t i = 0; i < num_cores; i++) {
        if (i != primary) {
...
void NavUKF::resetCoreErrors(void)
{
    for (uint8_t i = 0; i < num_cores; i++) {
...
void NavUKF::setPosVelYawSourceSet(uint8_t source_set_idx)
{
...
bool NavUKF::healthy(void) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::pre_arm_check(bool requires_position, char *failure_msg, uint8_t failure_msg_len) const
{
    ...
    // check if using compass (i.e. UKF_SRCn_YAW) with deprecated MAG_CAL values (5 was EXTERNAL_YAW, 6 was EXTERNAL_YAW_FALLBACK)
    ...
    for (uint8_t i = 0; i < num_cores; i++) {
        ...
        if (((magCalParamVal == 5) || (magCalParamVal == 6)) && (yaw_source != AP_NavEKF_Source::SourceYaw::GPS)) {
            ...
            dal.snprintf(failure_msg, failure_msg_len, "UKF_MAG_CAL and UKF_SRC1_YAW inconsistent");
            return false;
        }
    }

    if (!core) {
        dal.snprintf(failure_msg, failure_msg_len, "no UKF cores");
        return false;
    }
    for (uint8_t i = 0; i < num_cores; i++) {
        if (!core[i].healthy()) {
            ...
            if (failure != nullptr) {
                ...
                dal.snprintf(failure_msg, failure_msg_len, "UKF core %d unhealthy", (int)i);
            }
            return false;
        }
...
int8_t NavUKF::getPrimaryCoreIndex(void) const
{
    if (!core) {
        return -1;
    }
...
// return -1 if no primary core selected
int8_t NavUKF::getPrimaryCoreIMUIndex(void) const
{
    if (!core) {
        return -1;
    }
...
bool NavUKF::getPosNE(Vector2p &posNE) const
{
    if (!core) {
        return false;
    }
...
// If a calculated solution is not available, use the best available data and return false
// If false returned, do not use for flight control
bool NavUKF::getPosD(postype_t &posD) const
{
    if (!core) {
        return false;
    }
...
void NavUKF::getVelNED(Vector3f &vel) const
{
    if (core) {
...
bool NavUKF::getAirSpdVec(Vector3f &vel) const
{
    if (core) {
...
bool NavUKF::getAirSpdHealthData(uint8_t instance, float &innovation, float &innovationVariance, uint32_t &age_ms) const
{
    if (core == nullptr) {
        return false;
    }
...
float NavUKF::getPosDownDerivative() const
{
...
void NavUKF::getGyroBias(int8_t instance, Vector3f &gyroBias) const
{
...
void NavUKF::getAccelBias(int8_t instance, Vector3f &accelBias) const
{
    if (instance < 0 || instance >= num_cores) instance = primary;
    if (core) {
...
uint8_t NavUKF::get_active_source_set() const
{
    return sources.get_active_source_set();
}

// reset body axis gyro bias estimates
void NavUKF::resetGyroBias(void)
{
...
bool NavUKF::resetHeightDatum(void)
{
...
void NavUKF::getEkfControlLimits(float &ekfGndSpdLimit, float &ekfNavVelGainScaler) const
{
    if (core) {
...
bool NavUKF::getWind(Vector3f &wind) const
{
    if (core == nullptr) {
        return false;
    }
...
void NavUKF::getMagNED(Vector3f &magNED) const
{
    if (core) {
...
void NavUKF::getMagXYZ(Vector3f &magXYZ) const
{
    if (core) {
...
uint8_t NavUKF::getActiveAirspeed() const
{
    if (core) {
...
bool NavUKF::getMagOffsets(uint8_t mag_idx, Vector3f &magOffsets) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::getLLH(Location &loc) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::getOriginLLH(Location &loc) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::setOriginLLH(const Location &loc)
{
...
bool NavUKF::setLatLng(const Location &loc, float posAccuracy, uint32_t timestamp_ms)
{
...
bool NavUKF::getHAGL(float &HAGL) const
{
    if (!core) {
        return false;
    }
...
void NavUKF::getEulerAngles(Vector3f &eulers) const
{
    if (core) {
...
void NavUKF::getRotationBodyToNED(Matrix3f &mat) const
{
    if (core) {
...
void NavUKF::getQuaternionBodyToNED(int8_t instance, Quaternion &quat) const
{
    if (instance < 0 || instance >= num_cores) instance = primary;
    if (core) {
        Matrix3f mat;
...
void NavUKF::getQuaternion(Quaternion &quat) const
{
    if (core) {
...
bool NavUKF::getInnovations(Vector3f &velInnov, Vector3f &posInnov, Vector3f &magInnov, float &tasInnov, float &yawInnov) const
{
    if (core == nullptr) {
        return false;
    }
...
bool NavUKF::getVariances(float &velVar, float &posVar, float &hgtVar, Vector3f &magVar, float &tasVar, Vector2f &offset) const
{
    if (core == nullptr) {
        return false;
    }
...
bool NavUKF::getPosVelUncertainty(float &pos_horiz_m, float &pos_vert_m, float &vel_m_s) const
{
    if (core == nullptr) {
        return false;
    }
...
bool NavUKF::getVelInnovationsAndVariancesForSource(AP_NavEKF_Source::SourceXY source, Vector3f &innovations, Vector3f &variances) const
{
    if (core) {
...
bool NavUKF::use_compass(void) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::using_noncompass_for_yaw(void) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::using_extnav_for_yaw() const
{
    if (!core) {
        return false;
    }
...
// are we using a gps?
bool NavUKF::using_gps(void) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::configuredToUseGPSForPosXY(void) const
{
...
// check if configured to use GPS for horizontal position estimation
bool NavUKF::configuredToUseGPSForPos(void) const
{
...
void NavUKF::writeOptFlowMeas(const uint8_t rawFlowQuality, const Vector2f &rawFlowRates, const Vector2f &rawGyroRates, const uint32_t msecFlowMeas, const Vector3f &posOffset, float heightOverride)
{
...
bool NavUKF::getOptFlowSample(uint32_t& timeStamp_ms, Vector2f& flowRate, Vector2f& bodyRate, Vector2f& losPred) const
{
...
void NavUKF::writeEulerYawAngle(float yawAngle, float yawAngleErr, uint32_t timeStamp_ms, uint8_t type)
{
 ...
 *
*/
void NavUKF::writeExtNavData(const Vector3f &pos, const Quaternion &quat, float posErr, float angErr, uint32_t timeStamp_ms, uint16_t delay_ms, uint32_t resetTime_ms)
{
 ...
 * delay_ms : average delay of external nav system measurements relative to inertial measurements
*/
void NavUKF::writeExtNavVelData(const Vector3f &vel, float err, uint32_t timeStamp_ms, uint16_t delay_ms)
{
...
*/
void NavUKF::writeBodyFrameOdom(float quality, const Vector3f &delPos, const Vector3f &delAng, float delTime, uint32_t timeStamp_ms, uint16_t delay_ms, const Vector3f &posOffset)
{
...
*/
void NavUKF::writeWheelOdom(float delAng, float delTime, uint32_t timeStamp_ms, const Vector3f &posOffset, float radius)
{
 ...
 */
void NavUKF::writeTerrainData(float alt_m)
{
...
// parameter conversion of UKF parameters
void NavUKF::convert_parameters()
{
    ...
    // use UKF_GPS_TYPE to set UKF_SRC1_POSXY, UKF_SRC1_VELXY, UKF_SRC1_VELZ
    const AP_Param::ConversionInfo gps_type_info = {k_param_ekf3, 1, AP_PARAM_INT8, "UKF_GPS_TYPE"};
    ...
    if (found_gps_type) {
        ...
        case 0:
            // UKF_GPS_TYPE == 0 (GPS 3D Vel and 2D Pos)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
            AP_Param::set_and_save_by_name("UKF_SRC1_VELXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
            AP_Param::set_and_save_by_name("UKF_SRC1_VELZ", (int8_t)AP_NavEKF_Source::SourceZ::GPS);
            break;
        case 1:
            // UKF_GPS_TYPE == 1 (GPS 2D Vel and 2D Pos) then UKF_SRC1_POSXY = GPS(1), UKF_SRC1_VELXY = GPS(1), UKF_SRC1_VELZ = NONE(0)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
            AP_Param::set_and_save_by_name("UKF_SRC1_VELXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
            AP_Param::set_and_save_by_name("UKF_SRC1_VELZ", (int8_t)AP_NavEKF_Source::SourceZ::NONE);
            break;
        case 2:
            // UKF_GPS_TYPE == 2 (GPS 2D Pos) then UKF_SRC1_POSXY = GPS(1), UKF_SRC1_VELXY = None(0), UKF_SRC1_VELZ = NONE(0)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
            AP_Param::set_and_save_by_name("UKF_SRC1_VELXY", (int8_t)AP_NavEKF_Source::SourceXY::NONE);
            AP_Param::set_and_save_by_name("UKF_SRC1_VELZ", (int8_t)AP_NavEKF_Source::SourceZ::NONE);
            break;
        case 3:
        default:
            // UKF_GPS_TYPE == 3 (No GPS) we don't know what to do, could be optical flow, beacon or external nav
    ...
    // use UKF_ALT_SOURCE to set UKF_SRC1_POSZ
    const AP_Param::ConversionInfo alt_source_info = {k_param_ekf3, 9, AP_PARAM_INT8, "UKF_ALT_SOURCE"};
    AP_Int8 alt_source_old;
    if (AP_Param::find_old_parameter(&alt_source_info, &alt_source_old)) {
        switch (alt_source_old.get()) {
        case 0:
            // UKF_ALT_SOURCE = BARO, the default so do nothing
            break;
        case 1:
            // UKF_ALT_SOURCE == 1 (RangeFinder)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSZ", (int8_t)AP_NavEKF_Source::SourceZ::RANGEFINDER);
            break;
        case 2:
            // UKF_ALT_SOURCE == 2 (GPS)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSZ", (int8_t)AP_NavEKF_Source::SourceZ::GPS);
            break;
        case 3:
            // UKF_ALT_SOURCE == 3 (Beacon)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSZ", (int8_t)AP_NavEKF_Source::SourceZ::BEACON);
            break;
        case 4:
            // UKF_ALT_SOURCE == 4 (ExtNav)
            AP_Param::set_and_save_by_name("UKF_SRC1_POSZ", (int8_t)AP_NavEKF_Source::SourceZ::EXTNAV);
            break;
        default:
            // do nothing
            break;
    ...
    // use UKF_MAG_CAL to set UKF_SRC1_YAW
    switch (_magCal.get()) {
    case 5:
        // UKF_MAG_CAL = 5 (External Yaw sensor).  We rely on effective_magCal to interpret old "5" values as "Never"
        AP_Param::set_and_save_by_name("UKF_SRC1_YAW", (int8_t)AP_NavEKF_Source::SourceYaw::GPS);
        break;
    case 6:
        // UKF_MAG_CAL = 6 (ExtYaw with Compass fallback).  We rely on effective_magCal to interpret old "6" values as "When Flying"
        AP_Param::set_and_save_by_name("UKF_SRC1_YAW", (int8_t)AP_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK);
        break;
    default:
        // do nothing
        break;
    ...
    // if GPS and optical flow enabled set UKF_SRC2_VELXY to optical flow
    // UKF_SRC_OPTIONS should default to 1 meaning both GPS and optical flow velocities will be fused
    if (dal.opticalflow_enabled() && (!found_gps_type || (gps_type_old.get() <= 2))) {
        AP_Param::set_and_save_by_name("UKF_SRC2_VELXY", (int8_t)AP_NavEKF_Source::SourceXY::OPTFLOW);
    }
}
...
// enabled by the combination of UKF_RNG_USE_HGT and UKF_RNG_USE_SPD parameters.
void NavUKF::setTerrainHgtStable(bool val)
{
    if (val) {
...
*/
void NavUKF::getFilterFaults(uint16_t &faults) const
{
    if (core) {
...
*/
void NavUKF::getFilterStatus(nav_filter_status &status) const
{
    if (core) {
...
bool NavUKF::getTerrainAltVariance(float &terrainAltVar) const
{
    if (core) {
...
bool NavUKF::getHeightControlLimit(float &height) const
{
    if (!core) {
        return false;
    }
...
uint16_t NavUKF::getPosNorthEastResetCount(void)
{
    if (!core) {
        return 0;
    }
...
uint16_t NavUKF::getPosDownResetCount(void)
{
    if (!core) {
        return 0;
    }
...
uint16_t NavUKF::getYawResetCount(void)
{
    if (!core) {
        return 0;
    }
...
void NavUKF::updateLaneSwitchYawResetData(uint8_t new_primary)
{
...
void NavUKF::updateLaneSwitchPosResetData(uint8_t new_primary)
{
    // the new primary's historic in-core resets are not new events
    // for consumers; re-seat the count and record a single event for
    // the switch itself
...
void NavUKF::updateLaneSwitchPosDownResetData(uint8_t new_primary)
{
    // the new primary's historic in-core resets are not new events
    // for consumers; re-seat the count and record a single event for
    // the switch itself
...
void NavUKF::writeDefaultAirSpeed(float airspeed, float uncertainty)
{
...
bool NavUKF::yawAlignmentComplete(void) const
{
    if (!core) {
        return false;
    }
...
bool NavUKF::isVibrationAffected() const
{
    if (core) {
...
const EKFGSF_yaw *NavUKF::get_yawEstimator(void) const
{
    if (core) {
...
bool NavUKF::InitialiseFilterBootstrap()
{
    // ignore any data if the EKF is not started
    if (!core) {
        return false;
