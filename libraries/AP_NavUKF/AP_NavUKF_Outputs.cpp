...
bool NavUKF_core::healthy(void) const
{
    uint16_t faultInt;
...
*/
bool NavUKF_core::pre_arm_check(bool requires_position, char *failure_msg, uint8_t failure_msg_len) const
{
...
float NavUKF_core::errorScore() const
{
    float score = 0.0f;
...
bool NavUKF_core::getHeightControlLimit(float &height) const
{
...
void NavUKF_core::getEulerAngles(Vector3f &euler) const
{
...
void NavUKF_core::getGyroBias(Vector3f &gyroBias) const
{
    if (dtEkfAvg < 1e-6f) {
        gyroBias.zero();
        return;
...
void NavUKF_core::getAccelBias(Vector3f &accelBias) const
{
...
void NavUKF_core::getRotationBodyToNED(Matrix3f &mat) const
{
...
void NavUKF_core::getQuaternion(Quaternion& ret) const
{
...
bool NavUKF_core::getWind(Vector3f &wind) const
{
...
//
void NavUKF_core::getVelNED(Vector3f &vel) const
{
...
bool NavUKF_core::getAirSpdVec(Vector3f &vel) const
{
...
bool NavUKF_core::getAirSpdHealthData(float &innovation, float &innovationVariance, uint32_t &age_ms) const
{
...
float NavUKF_core::getPosDownDerivative(void) const
{
...
bool NavUKF_core::getPosNE(Vector2p &posNE) const
{
...
// Return true if the estimate is valid
bool NavUKF_core::getPosD_local(postype_t &posD) const
{
...
// Return true if the estimate is valid
bool NavUKF_core::getPosD(postype_t &posD) const
{
...
bool NavUKF_core::getHAGL(float &HAGL) const
{
...
bool NavUKF_core::getLLH(Location &loc) const
{
    Location origin;
...
bool NavUKF_core::getGPSLLH(Location &loc) const
{
...
void NavUKF_core::getEkfControlLimits(float &ekfGndSpdLimit, float &ekfNavVelGainScaler) const
{
...
bool NavUKF_core::getOriginLLH(Location &loc) const
{
    if (validOrigin) {
        loc = public_origin;
...
void NavUKF_core::getMagNED(Vector3f &magNED) const
{
    magNED = (stateStruct.earth_magfield * 1000.0f).tofloat();
}

// return body magnetic field estimates in measurement units / 1000
void NavUKF_core::getMagXYZ(Vector3f &magXYZ) const
{
...
bool NavUKF_core::getMagOffsets(uint8_t mag_idx, Vector3f &magOffsets) const
{
...
uint8_t NavUKF_core::getActiveAirspeed() const
{
    return (uint8_t)selected_airspeed;
}

// return the innovations for the NED Pos, NED Vel, XYZ Mag and Vtas measurements
bool NavUKF_core::getInnovations(Vector3f &velInnov, Vector3f &posInnov, Vector3f &magInnov, float &tasInnov, float &yawInnov) const
{
...
void NavUKF_core::getSynthAirDataInnovations(Vector2f &dragInnov, float &betaInnov) const
{
...
bool NavUKF_core::getVariances(float &velVar, float &posVar, float &hgtVar, Vector3f &magVar, float &tasVar, Vector2f &offset) const
{
...
bool NavUKF_core::getPosVelUncertainty(float &pos_horiz_m, float &pos_vert_m, float &vel_m_s) const
{
    if (!statesInitialised) {
        return false;
    }
...
bool NavUKF_core::getVelInnovationsAndVariancesForSource(AP_NavEKF_Source::SourceXY source, Vector3f &innovations, Vector3f &variances) const
{
    switch (source) {
...
*/
void  NavUKF_core::getFilterFaults(uint16_t &faults) const
{
...
void  NavUKF_core::getFilterStatus(nav_filter_status &status) const
{
    status = filterStatus;
}

// return a terrain altitude variance
bool NavUKF_core::getTerrainAltVariance(float &temp) const
{
...
const char *NavUKF_core::prearm_failure_reason(void) const
{
    if (gpsGoodToAlign) {
        // we are not failing
        return nullptr;
...
uint8_t NavUKF_core::getFramesSincePredict(void) const
{
