...
void NavUKF_core::controlFilterModes()
{
 ...
 */
NavUKF_core::MagCal NavUKF_core::effective_magCal(void) const
{
...
// Determine if learning of wind and magnetic field will be enabled and set corresponding indexing limits to
// avoid unnecessary operations
void NavUKF_core::setWindMagStateLearningMode()
{
...
void NavUKF_core::updateStateIndexLim()
{
...
void NavUKF_core::setAidingMode()
{
    ...
    if (PV_AidingMode != PV_AidingModePrev) {
        ...
        case AID_NONE:
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "UKF IMU%u stopped aiding",(unsigned)imu_index);
        ...
        case AID_RELATIVE:
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u started relative aiding",(unsigned)imu_index);
#if UKF_FEATURE_OPTFLOW_FUSION
        ...
        case AID_ABSOLUTE:
            if (readyToUseGPS()) {
                ...
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u is using GPS",(unsigned)imu_index);
#if UKF_FEATURE_BEACON_FUSION
            } else if (readyToUseRangeBeacon()) {
                // We are commencing aiding using range beacons
                posResetSource = resetDataSource::RNGBCN;
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u is using range beacons",(unsigned)imu_index);
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u initial pos NE = %3.1f,%3.1f (m)",(unsigned)imu_index,(double)rngBcn.receiverPos.x,(double)rngBcn.receiverPos.y);
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u initial beacon pos D offset = %3.1f (m)",(unsigned)imu_index,(double)rngBcn.posOffsetNED.z);
                ...
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u is using external nav data",(unsigned)imu_index);
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u initial pos NED = %3.1f,%3.1f,%3.1f (m)",(unsigned)imu_index,(double)extNavDataDelayed.pos.x,(double)extNavDataDelayed.pos.y,(double)extNavDataDelayed.pos.z);
                if (useExtNavVel) {
                    velResetSource = resetDataSource::EXTNAV;
                    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u initial vel NED = %3.1f,%3.1f,%3.1f (m/s)",(unsigned)imu_index,(double)extNavVelDelayed.vel.x,(double)extNavVelDelayed.vel.y,(double)extNavVelDelayed.vel.z);
                }
...
// Used during initial bootstrap alignment of the filter
void NavUKF_core::checkAttitudeAlignmentStatus()
{
    ...
    if (!tiltAlignComplete) {
        if (tiltErrorVariance < sq(radians(5.0))) {
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u tilt alignment complete",(unsigned)imu_index);
        }
    }
...
bool NavUKF_core::useAirspeed(void) const
{
    return dal.airspeed_sensor_enabled();
}

// return true if we should use the range finder sensor
bool NavUKF_core::useRngFinder(void) const
{
...
bool NavUKF_core::readyToUseOptFlow(void) const
{
...
bool NavUKF_core::readyToUseBodyOdm(void) const
{
...
bool NavUKF_core::readyToUseGPS(void) const
{
...
bool NavUKF_core::readyToUseRangeBeacon(void) const
{
#if UKF_FEATURE_BEACON_FUSION
...
bool NavUKF_core::readyToUseExtNav(void) const
{
#if UKF_FEATURE_EXTERNAL_NAV
...
bool NavUKF_core::use_compass(void) const
{
...
bool NavUKF_core::using_noncompass_for_yaw(void) const
{
#if UKF_FEATURE_EXTERNAL_NAV
...
bool NavUKF_core::using_extnav_for_yaw() const
{
#if UKF_FEATURE_EXTERNAL_NAV
    if (yaw_source_last == AP_NavEKF_Source::SourceYaw::EXTNAV) {
        return ((imuSampleTime_ms - last_extnav_yaw_fusion_ms < 5000) || (imuSampleTime_ms - lastSynthYawTime_ms < 5000));
...
// are we using a gps
bool NavUKF_core::using_gps() const
{
 ...
 */
bool NavUKF_core::assume_zero_sideslip(void) const
{
...
void NavUKF_core::setEarthFieldFromLocation(const Location &loc)
{
    const auto &compass = dal.compass();
...
bool NavUKF_core::setOriginLLH(const Location &loc)
{
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "UKF IMU%u origin set",(unsigned)imu_index);
...
void NavUKF_core::recordYawResetsCompleted()
{
...
void NavUKF_core::checkGyroCalStatus(void)
{
...
void  NavUKF_core::updateFilterStatus(void)
{
    // init return value
    ...
    status.flags.pred_horiz_pos_rel = status.flags.horiz_pos_rel; // UKF enters the required mode before flight
    status.flags.pred_horiz_pos_abs = status.flags.horiz_pos_abs; // UKF enters the required mode before flight
...
void NavUKF_core::runYawEstimatorPrediction()
{
...
void NavUKF_core::runYawEstimatorCorrection()
{
    // exit immediately if no yaw estimator
    if (yawEstimator == nullptr) {
        return;
...
void NavUKF_core::EKFGSF_requestYawReset()
{
