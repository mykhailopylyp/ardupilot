...
NavUKF AP_AHRS_NavUKF::UKF;

bool AP_AHRS_NavUKF::start()
{
    ...
    return UKF.InitialiseFilter();
}

void AP_AHRS_NavUKF::update()
{
    if (!started) {
        started = start();
    }
    ...
    const int8_t primary_core = UKF.getPrimaryCoreIndex();
    if (attitude_reset_tracker.update(primary_core)) {
        LOGGER_WRITE_ERROR(LogErrorSubsystem::EKF_PRIMARY, LogErrorCode(primary_core));
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "UKF primary changed:%d", (unsigned)primary_core);
    }

    yaw_reset_tracker.update(UKF.getYawResetCount());

    position_NE_reset_tracker.update(UKF.getPosNorthEastResetCount());

    position_D_reset_tracker.update(UKF.getPosDownResetCount());
}

void AP_AHRS_NavUKF::get_results(AP_AHRS_Backend::Estimates &results)
{
    const auto now_ms = AP_HAL::millis();
    ...
    results.healthy = started && UKF.healthy();
    ...
    const int8_t ukf_primary_gyro = UKF.getPrimaryCoreIMUIndex();
    if (ukf_primary_gyro == -1) {
        results.primary_gyro = _ins.get_first_usable_gyro();
    } else {
        results.primary_gyro = ukf_primary_gyro;
    }

    // get gyro bias for primary EKF and change sign to give gyro drift
    // Note sign convention used by EKF is bias = measurement - truth
    Vector3f drift;
    UKF.getGyroBias(-1, drift);
     ...
     * attitude estimates:
     */
    UKF.getRotationBodyToNED(results.dcm_matrix);

    Vector3f eulers;
    UKF.getEulerAngles(eulers);
    results.roll_rad  = eulers.x;
    results.pitch_rad = eulers.y;
    results.yaw_rad   = eulers.z;

    UKF.getQuaternion(results.quaternion);
    ...
    const int8_t ukf_primary_accel = UKF.getPrimaryCoreIMUIndex();
    if (ukf_primary_accel == -1) {
        results.primary_accel = _ins.get_first_usable_accel();
    } else {
        results.primary_accel = ukf_primary_accel;
    }

    // get 3-axis accel bias estimates for active EKF (this is usually
    // for the primary IMU)
    UKF.getAccelBias(-1, results.accel_bias);
     ...
     * velocity estimates
     */
    UKF.getVelNED(results.velocity_NED);
    results.velocity_NED_valid = true;
    results.vert_pos_rate_D = UKF.getPosDownDerivative();
     ...
     * position estimates
     */
    results.location_valid = UKF.getLLH(results.location);

    // origin-relative functions
    results.provides_common_origin = true;

    // origin-relative position:
    results.position_NE_valid = UKF.getPosNE(results.position_NE);
    results.position_NE_reset_count = position_NE_reset_tracker.count();

    results.position_D_valid = UKF.getPosD(results.position_D);
    results.position_D_reset_count = position_D_reset_tracker.count();

    results.hagl_valid = UKF.getHAGL(results.hagl);

    /*
     * air data estimates
     */
    results.wind_valid = UKF.getWind(results.wind);
    ...
    results.configured_to_use_gps = UKF.using_gps();
    ...
    results.configured_to_use_gps_for_pos_XY = UKF.configuredToUseGPSForPosXY();

    // are we consuming yaw from an external (e.g. vision-based) source?
    results.using_extnav_for_yaw = UKF.using_extnav_for_yaw();

    // are we consuming yaw from a source which is *not* a compass
    // (e.g. the GSF)
    results.using_noncompass_for_yaw = UKF.using_noncompass_for_yaw();

#if AP_AHRS_GET_MAG_DATA_ENABLED
    // estimators can provide their predicted magnetic fields:
    UKF.getMagNED(results.mag_field_NED);
    results.mag_field_NED_valid = true;
    UKF.getMagXYZ(results.mag_field_corrections);
     ...
     */
    UKF.getFilterStatus(results.filter_status);
    results.filter_status_valid = true;

    UKF.getFilterFaults(results.filter_faults);

    // provides the innovations normalised between 0 and 1:
    Vector2f offset;
    results.variances_valid = UKF.getVariances(results.velVar, results.posVar, results.hgtVar, results.magVar, results.tasVar, offset);

    results.terrain_alt_variance_valid = UKF.getTerrainAltVariance(results.terrain_alt_variance);
}

bool AP_AHRS_NavUKF::pre_arm_check(bool requires_position, char *failure_msg, uint8_t failure_msg_len) const
{
    if (!started) {
        hal.util->snprintf(failure_msg, failure_msg_len, "UKF not started");
        return false;
    }
    return UKF.pre_arm_check(requires_position, failure_msg, failure_msg_len);
}
